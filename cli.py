"""
Command Line Interface for Ecmo Cannulation Va Vv Agent.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="ecmo-cannulation-va-vv-agent", description="Ecmo Cannulation Va Vv Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  ECMO CANNULATION VA VV AGENT")
    ...
    if args.command == "chat":
        ...
        print(f"\n[Ecmo Cannulation Va Vv Agent Supervisor]:\n{ans}\n")
        return 0
        ...
        for idx, r in enumerate(rows):
            try:
                payload = SystemTaskPayload(
                    ...
                    primary_metric=float(r.get("primary_metric", 15.0) or 15.0),
                    secondary_metric=float(r.get("secondary_metric", 5.0) or 5.0),
    ...
    if args.command == "serve":
        ...
        print(f"Starting Ecmo Cannulation Va Vv Agent API server on http://{args.host}:{args.port}")
